using MarkEdit.Core;
using MarkEdit.Core.Commands;

namespace MarkEdit.Commands.Search;

public class FindNextCommand : ICommand
{
    private readonly ITextEditor _editor;
    private readonly SearchContext _context;

    public FindNextCommand(ITextEditor editor, SearchContext context)
    {
        _editor = editor;
        _context = context;
    }

    public void Execute()
    {
        if (string.IsNullOrEmpty(_context.CurrentTerm))
        {
            return;
        }

        var start = _context.LastMatchIndex + 1;
        var index = _editor.Text.IndexOf(_context.CurrentTerm, start, StringComparison.OrdinalIgnoreCase);
        if (index >= 0)
        {
            _editor.Select(index, _context.CurrentTerm.Length);
            _editor.ScrollToCaret();
            _context.LastMatchIndex = index;
        }
    }

    public bool CanExecute()
        => !string.IsNullOrEmpty(_context.CurrentTerm);
}